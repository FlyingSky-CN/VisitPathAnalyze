...
if (!file_exists(DIR_config.'/theme/echarts-bar-1.html')) 
exit('fatal: theme file \'echarts-bar-1.html\' did not exists.'."\n");

if (!($theme = file_get_contents(DIR_config.'/theme/echarts-bar-1.html')))
exit('fatal: can not read the theme file.'."\n");

if (!isset($argv[3]))
exit('usage: echarts-bar-1 <input> [<title>]'."\n");
